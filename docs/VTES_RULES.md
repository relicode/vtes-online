# Vampire: The Eternal Struggle (VTES) -- Rules Reference

## 1. Game Overview

Vampire: The Eternal Struggle is a multiplayer customizable card game set in the World of Darkness. Players take the role of ancient vampires called **Methuselahs** who manipulate younger vampires (and other minions) to destroy their rivals.

- **Player count:** 4-5 (playable with 2-6)
- **Seating:** The player to your **left** is your **prey**; the player to your **right** is your **predator**. Bleeds target your prey, creating a clockwise chain of aggression.
- **Golden Rule:** Card text overrides these rules whenever they conflict.

---

## 2. Card Types

Two separate decks: **Crypt** (vampires/imbued) and **Library** (everything else).
...
- Cannot be targeted by vampire-specific effects
- At 0 life, go to incapacitated region (not torpor); recover by burning 3 conviction during unlock phase
...
- Gained from: hunting (+1), card effects, diablerie
...
- **Referendums:** Can be burned for +1 vote
- Can be burned for +1 bleed
...
1. Determine seating randomly, select random first player
2. Each player receives **30 pool** from blood bank
3. Shuffle crypt and library separately
4. Draw **7 library cards** into hand
5. Draw **4 crypt cards** face-down into uncontrolled region
6. Place Edge uncontrolled in center
...
- **Ash Heap:** Discard pile (visible to all)
...
Five phases per turn, clockwise order.

### 5.1 Unlock Phase

1. Unlock all your cards
2. Edge benefit: gain 1 pool if you hold the Edge
3. Contested cards: pay 1 pool each or yield
4. Contested titles: vampire pays 1 blood or yields (torpored vampires must yield)
5. Conviction: play 1 conviction per imbued (from hand or ash heap)
6. Burn option: discard 1 burn-option card if no ready minion meets requirements

### 5.2 Master Phase

- 1 master phase action by default (reduced by out-of-turn masters played since last master phase)
- Play a master card from hand
- Trifle grants +1 master phase action
- Unused actions lost

### 5.3 Minion Phase

Ready, unlocked minions perform actions. **Taking an action locks the minion.** One action per minion per turn (unless card grants more).
...
- 1 discard phase action by default
- Discard a card and draw replacement, OR play an Event card
- Max 1 event per discard phase; each event once per game globally
...
1. **Announce:** Declare action, name target, play action card (set aside), lock acting minion
2. **Block attempts:**
   - **Directed actions:** Only targeted Methuselah's minions may block
   - **Undirected actions:** Prey gets first block opportunity, then predator
   - Blocker declared -> compare stealth vs intercept
   - Block succeeds: blocker locked, combat begins, action fails, action card burned
   - Block fails: may try different minion
   - All decline/fail: action succeeds
3. **Resolve:** Pay costs, apply effects

### 6.2 Stealth and Intercept

- Default: both 0 (unless action type specifies otherwise)
- Block **succeeds** if intercept >= stealth
- Block **fails** if stealth > intercept
- Stealth increased only when currently blocked; intercept increased only when currently unable to block
- Modifications last for current action only

### 6.3 Impulse Sequencing

1. Acting Methuselah has first impulse
2. After passing, impulse moves to defender/prey/predator then clockwise
3. If anyone plays a card, acting Methuselah regains impulse
4. Resolves when all pass in sequence

---

## 7. Combat

Occurs when an action is blocked or forced by card effect. One or more rounds:

### 7.1 Combat Round

1. **Before Range:** Play "before range" effects
2. **Determine Range:** Default close. Maneuvers change range. Alternate between combatants.
3. **Before Strikes:** Play "before strikes" effects
4. **Choose Strikes:** Acting minion first, then opponent. Resolve simultaneously (unless First Strike).
5. **Damage Resolution:** Prevent -> Apply -> Mend
6. **Press:** Continue (another round) or end. Alternate like maneuvers.
7. **End of Round:** "At end of round" effects
...
**Normal damage to vampires:** Burn blood to mend, 1 blood per damage. Cannot mend all -> wounded -> torpor at 0 blood.

**Normal damage to allies/retainers:** Burn 1 life per damage. At 0 life, burned.

**Aggravated damage to vampires:** Cannot be mended. Immediately wounds. Wounded vampire must burn 1 blood per aggravated point to survive; otherwise burned (destroyed). Normal damage applied before aggravated when simultaneous.

**Aggravated damage to allies/retainers:** Same as normal (burn life counters).

### 7.4 Torpor

- Face-up, controlled, not ready
- Cannot act (except leave torpor), block, or play reactions
- Can play action modifiers during own actions
- Cannot cast votes
- Unlock normally
- Must yield contested titles

**Leaving torpor:** Action costing 2 blood. Ready vampire can rescue (2 blood, shareable).

### 7.5 Diablerie

1. Action succeeds -> all victim's blood moves to diablerist (excess drains)
2. Diablerist may take victim's equipment
3. Victim burned
4. If victim had higher capacity: diablerist may search for master Discipline card
5. Blood Hunt referendum automatically called -- if passes, diablerist burned

---

## 8. Political Actions and Referendums

### 8.1 Referendum Process

1. **Choose Terms:** Announce specific targets/effects
2. **Polling:** "Before votes" effects, then all Methuselahs cast votes/ballots freely. Once cast, cannot change.
3. **Resolve:** More votes in favor -> passes. Ties -> fails.
